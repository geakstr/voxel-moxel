...
public class Chunk extends Mesh {
	...
	public Chunk(int x_chunk_pos, int y_chunk_pos, int z_chunk_pos) {
		...
		this.vertices_size = World.chunk_volume * Cube.cube_side_vertices_size
				* 6;
		this.textures_size = World.chunk_volume * Cube.cube_side_texture_size
				* 6;
		this.textures_offsets_size = World.chunk_volume
				* Cube.cube_side_texture_size * 6;
	...
	public void update() {
		...
		for (int z = 0; z < World.chunk_height; z++) {
				...
				for (int x = 0; x < World.chunk_width; x++) {
					...
					if (proj[x] != -1) {
						...
						update_coords = true;
							...
							&& (Cube.unpack_type(cubes[x - 1][y][z]) == type) && (projFlag != x - 1))) {
						mark[y][x] = mark[y][x - 1];
						update_coords = true;
					} else {
						mark[y][x] = ++next_color;
					...
					canDown = !(len > 0 && !canDown)
							&& (y < (World.chunk_length - 1) && (Cube
									.unpack_type(cubes[x][y + 1][z]) == type));
			...
			for (Map.Entry<Integer, int[]> e : coords_map.entrySet()) {
				...
				Vector2f tex = rnd.nextBoolean() ? TextureAtlas
						.get_coord("grass") : TextureAtlas.get_coord("dirt");
				for (int side_idx = 0; side_idx < 6; side_idx++) {
					if (renderable_sides[side_idx]) {
						vertices.addAll(Arrays.asList(Cube.get_side(side_idx,
								x0 + x_offset, y0 + y_offset, x1 + x_offset, y1
										+ y_offset, z + z_offset)));
						textures.addAll(Arrays.asList(Cube.get_texture(
								side_idx, x0, y0, x1, y1)));
						textures_offsets.addAll(Arrays.asList(tex.x, tex.y,
								tex.x, tex.y, tex.x, tex.y, tex.x, tex.y,
								tex.x, tex.y, tex.x, tex.y));
						...
						colors.addAll(Arrays.asList(r, g, b, r, g, b, r, g, b,
								r, g, b, r, g, b, r, g, b));
					}
				}
			}
	...
	public boolean[] renderable_sides(int x0, int y0, int x1, int y1, int z) {
		...
		if (sides[1] && x1 == World.chunk_width - 1
		...
		if (sides[2] && y1 == World.chunk_length - 1
		...
		if (sides[5] && z == World.chunk_height - 1
	...
	public void occlusion_render() {
		...
		//return glGetQueryObjectui(occlusion_query, GL_QUERY_RESULT_ARB) > 0;
	}
}
